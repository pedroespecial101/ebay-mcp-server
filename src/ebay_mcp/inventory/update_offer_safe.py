        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                try:
                    ...
                    if 'effective_offer_id' in locals():
                        error_message = f"Error updating offer {effective_offer_id}: {e.response.status_code} - {e.response.text}"
                    else:
                        error_message = f"Error fetching offer details for '{params.sku or params.offer_id}': {e.response.status_code} - {e.response.text}"
            ...
            error_msg = (
                "Critical error in update_offer_safe setup or "
                f"pre-API call phase: {str(e)}"
            )
